...
def preprocessMessage(tweets_list):
    ...
    for tweet in tweets_list:
        ...
        # Remove emails from the tweets, if any.
        temp.append(re.sub('\S*@\S*\s?', "", tweet))

        # Remove new line characters from the tweets, if any.
        temp.append(re.sub('\s+', " ", tweet))

        # Remove distracting single quotes from the tweets, if any.
        temp.append(re.sub("\'", "", tweet))

        # Remove any URLs from the tweets, if any.
        temp.append(re.sub("http\S+", "", tweet))

    tweets_list = temp

    # Break the sentence(s) into individual tokens/words.
    tweets_list = tokenize(tweets_list)
...
def remove_stopwords(messages):
    return [[word for word in message if word not in stop_words] for message in messages]



def tokenize(messages):
    return [message.split() for message in messages]
